...
ba_api = "https://gs-bil-api.bluearchive-cn.com/api/state"

headers = {'APP-VER': '1.6.1', 'PLATFORM-ID': '1', 'CHANNEL-ID': '2'}


def getConfig():
    return requests.get(ba_api, headers=headers).json()
...
def getModelsList():
    ...
    '''
    data = []
    base_url = config["AddressablesCatalogUrlRoots"][0] + '/AssetBundles'
    ver = config["ResourceVersion"]
    res_url = base_url + '/Catalog/' + ver + '/Android/bundleDownloadInfo.json'
...
if __name__ == "__main__":
    ...
    config = getConfig()
    ver = config["ResourceVersion"] + '_' + config["MediaVersion"]
    print(ver)
